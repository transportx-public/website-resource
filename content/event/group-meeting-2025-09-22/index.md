---
title: "组会 | 2025-09-22"
event: "课题组组会"
summary: "本次组会收录 2 篇文献阅读分享。"
abstract: ''
date: "2025-09-22T00:00:00+08:00"
all_day: true
publishDate: "2025-09-22T00:00:00+08:00"
authors: []
tags:
  - "组会"
  - "文献分享"
featured: false
image:
  caption: ''
  focal_point: Center
url_code: ''
url_pdf: ''
url_slides: ''
url_video: ''
slides: ''
projects: []
meeting_slides: ""
meeting_notes: ""
reading_members:
  - name: "卢永晟"
    profile: "Yongsheng Lu"
  - name: "黄知微"
    profile: "Zhiwei Huang"
readings:
  - title: "ST-LINK: Spatially-Aware Large Language Models for Spatio-Temporal Forecasting"
    sequence: 34
    reader_name: "卢永晟"
    reader_profile: "Yongsheng Lu"
    published: "2025-09"
    source: "arXiv"
    highlight: "该文提出 ST-LINK 框架，通过空间增强注意力（SE-Attention）和记忆检索前馈网络（MRFFN），将空间感知融入大型语言模型，解决其在交通等时空预测中难以建模空间依赖的问题，且性能优于传统深度学习与现有 LLM 方法 。"
    url: ""
    slides: ""
  - title: "The sequential construction research of regional public electric vehicle charging facilities based on data-driven analysis-Empirical analysis of Shanxi Province"
    sequence: 35
    reader_name: "黄知微"
    reader_profile: "Zhiwei Huang"
    published: "2022-12"
    source: "Journal of Cleaner Production"
    highlight: "本研究提出了一种数据驱动的电动汽车公共充电设施建设时序确定方法，以解决供需失衡问题。本文利用山西省的实际充电数据，分析了城市间的需求差异和城市内的利用差异。"
    url: ""
    slides: ""
---
